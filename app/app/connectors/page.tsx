    ...
    <>
      ...
      <main className="px-6 lg:px-10 py-8 space-y-8 max-w-[1320px]">
        <div className="card p-5 flex items-start gap-4">
          ...
          <button className="btn btn-primary h-8 px-3 text-[12px]">Add connector</button>
        </div>
                ...
                <div key={c.id} className="card p-5 flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <div className="text-[13.5px] font-semibold tracking-tight truncate">{c.name}</div>
                      <span className={`text-[11px] font-medium ${statusStyle[c.status]}`}>{statusLabel[c.status]}</span>
                    </div>
                    {c.account && (
                      <div className="text-[11.5px] text-[color:var(--fg-subtle)] mt-0.5 truncate">{c.account}</div>
                    )}
                    <div className="mt-3 flex items-center justify-between text-[11.5px]">
                      <span className="text-[color:var(--fg-muted)]">
                        {c.agents} {c.agents === 1 ? "agent" : "agents"} using
                      </span>
                      {c.status === "not_configured" ? (
                        <button className="btn btn-secondary h-7 px-2.5 text-[11px]">Connect</button>
                      ) : c.status === "needs_reauth" || c.status === "error" ? (
                        <button className="btn btn-primary h-7 px-2.5 text-[11px]">Reconnect</button>
                      ) : (
                        <button className="btn btn-ghost h-7 px-2.5 text-[11px]">Manage</button>
                      )}
                    </div>
                  </div>
                </div>
