...
const nav = [
  ...
  { href: "/app/approvals", label: "Approvals", icon: "M9 12l2 2 4-4 M12 21a9 9 0 110-18 9 9 0 010 18z" },
    ...
    <aside className="hidden lg:flex fixed inset-y-0 left-0 w-60 flex-col border-r border-[color:var(--border)] bg-[color:var(--bg)]/85 backdrop-blur-xl z-30">
      ...
      <div className="px-3 pt-3 pb-2">
        <div className="rounded-xl border border-[color:var(--border)] bg-[color:var(--surface)] px-3 py-2.5 flex items-center justify-between">
          <div className="min-w-0">
            <div className="text-[10.5px] uppercase tracking-[0.18em] text-[color:var(--fg-subtle)] font-medium">Workspace</div>
            <div className="text-[13px] font-medium tracking-tight truncate">{workspaceName}</div>
          </div>
          <svg viewBox="0 0 16 16" className="h-3.5 w-3.5 text-[color:var(--fg-subtle)] flex-none" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 6l4 4 4-4" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </div>
      </div>
      ...
      <div className="p-3 border-t border-[color:var(--border)]">
        <div className="rounded-xl bg-gradient-to-br from-[color:var(--brand-soft)] to-[color:var(--surface-alt)] p-3.5 border border-[color:var(--border)]">
          <div className="text-[12px] font-medium tracking-tight">You're on the free plan</div>
          <div className="text-[11px] text-[color:var(--fg-muted)] mt-0.5">1 agent included</div>
        ...
        <div className="mt-3 flex items-center gap-2.5 px-2 py-1.5">
          <span className="h-7 w-7 rounded-full bg-gradient-to-br from-[color:var(--brand)] to-[color:var(--accent)] grid place-items-center text-[color:var(--on-brand)] text-[11px] font-semibold uppercase">
            {userName.slice(0, 1)}
          </span>
          <div className="min-w-0 flex-1">
            <div className="text-[12.5px] font-medium truncate">{userName}</div>
            <div className="text-[10.5px] text-[color:var(--fg-subtle)] truncate">{userEmail}</div>
          </div>
          <form action={signOutAction}>
            <button type="submit" className="text-[color:var(--fg-subtle)] hover:text-[color:var(--fg)]" title="Sign out">
